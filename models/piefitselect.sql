...
SELECT c.name, c.difficulty, c.video_url, c.type, rar.rounds, rar.repsandrounds_id from card c 
LEFT JOIN repsandrounds rar ON c.card_id = rar.card_id
WHERE c.card_id = 5;

-- exercises of single card of type repsandrounds
SELECT e.name, rare.reps from exercise e
...
ORDER BY c.card_id ASC, rarb.time DESC;

-- All cards of type amrap, with benchmark
SELECT c.name, c.type, b.benchmark_id as "#", b.level, ab.rounds 
from card c 
